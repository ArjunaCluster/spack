...
class Slurm(AutotoolsPackage):
    ...
    variant('pam', default=False, description='Enable PAM authentication')
    variant('nvml', default='/usr/local/cuda', description='Enable NVML support', values=any)
    # fmt: on
    ...
    def configure_args(self):
        ...
        if "+pam" in spec:
            args.append("--enable-pam")

        nvml_dir = spec.variants["nvml"].value
        if nvml_dir:
            args.append("--with-nvml={0}".format(nvml_dir))
